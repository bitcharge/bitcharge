class Global
  ...
  class << self
    ...
    def daemon_statuses
      Rails.cache.fetch('myexchangepea:daemons:statuses', expires_in: 3.minute) do
  ...
  def key(key, interval=5)
    ...
    "myexchangepea:#{@currency}:#{key}:#{time_key}"
  end

  def asks
    Rails.cache.read("myexchangepea:#{currency}:depth:asks") || []
  end

  def bids
    Rails.cache.read("myexchangepea:#{currency}:depth:bids") || []
  end
  ...
  def ticker
    ticker           = Rails.cache.read("myexchangepea:#{currency}:ticker") || default_ticker
    open = Rails.cache.read("myexchangepea:#{currency}:ticker:open") || ticker[:last]
  ...
  def trades
    Rails.cache.read("myexchangepea:#{currency}:trades") || []
  end
