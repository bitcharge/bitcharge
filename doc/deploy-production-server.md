...
##### Clone the Source
    ...
    mkdir -p ~/myexchangepea
    git clone git://github.com/myexchangepea/myexchangepea.git ~/myexchangepea/current
    cd myexchangepea/current
...
* Myexchangepea depends on [Pusher](http://pusher.com). A development key/secret pair for development/test is provided in `config/application.yml` (uncomment to use). PLEASE USE IT IN DEVELOPMENT/TEST ENVIRONMENT ONLY!
...
**Passenger:**
    ...
    sudo ln -s /home/deploy/myexchangepea/current/config/nginx.conf /etc/nginx/conf.d/myexchangepea.conf
